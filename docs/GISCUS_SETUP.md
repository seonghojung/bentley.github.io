...
1. GitHub 저장소 (`bentley.github.io`)로 이동
2. **Settings** 탭 클릭
...
3. 설치할 저장소 선택:
   ...
   - `bentley.github.io` 저장소 선택
4. **Install** 클릭

## 3단계: Giscus 설정 정보 가져오기

1. [https://giscus.app/ko](https://giscus.app/ko) 접속
2. **저장소** 섹션에서 `seonghojung/bentley.github.io` 입력
3. 설정 옵션:
...
2. `loadGiscus()` 함수에서 다음 값 업데이트:
   ...
   script.setAttribute("data-repo-id", "R_kgDOQSg8iA"); // 3단계에서 복사한 값으로 변경
   script.setAttribute("data-category", "General"); // 선택한 카테고리 이름
   script.setAttribute("data-category-id", "DIC_kwDOQSg8iM4CxnwT"); // 3단계에서 복사한 값으로 변경
   ```
